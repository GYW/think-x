package com.think.x.core.base.id;
...
public class SnowFlakeIdWorker implements IdentifierGenerator {
    /**
     * 开始时间截(2023-01-01 00:00:00) 毫秒级时间戳
     */
    ...
    /**
     * 支持的最大机器id，结果是7 (这个移位算法可以很快的计算出几位二进制数所能表示的最大十进制数) * -1L ^ (-1L << n)表示占n个bit的数字的最大值是多少。举个栗子：-1L ^ (-1L << 2)等于10进制的3 ，即二进制的11表示十进制3。
     */
    private final long maxWorkerId = ~(-1L << workerIdBits);
    /**
     * 支持的最大数据标识id，结果是7 * -1L 原码 1000 0001 原码是在符号位加标志 正数 0 负数1 * 反码1111 1110 正数原码是其本身 负数是符号位不变其余位取反 * 补码1111 1111 原码-1再取反 * -1L << 3 1111 1000 * -1L ^ (1111 1000) 1111 1000 ^ 1111 1111 结果为7 *
     */
    ...
    /**
     * 构造函数 * @param workerId 工作ID (0~7) * @param datacenterId 数据中心ID (0~7)
     */
